...
public class YellowPiece : PlayerPiece
{
     int playerIndex;
    GameDice Dice;
    Manager GameManager;
    ...
    public void OnMouseDown() {
       if(Manager.gm.whoseTurn == 4 && Manager.gm.canMove == true && hasLaunched == true){
            Debug.Log(Manager.gm.canMove);
            MovePlayer();
        }

        if(Manager.gm.whoseTurn == 4 && Manager.gm.canLaunch == true && hasLaunched == false){
            LaunchNewPiece();           
        }           
    }
    ...
    void MovePlayer(){
        StartCoroutine(MoveSteps(PlayerPath.PathY));
    }
